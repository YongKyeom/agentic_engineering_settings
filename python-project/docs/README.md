...
루트 `AGENTS.md`에는 모든 작업에 적용되는 공통 기준과 프로젝트별 기본값·지도를 둡니다.
세부 절차와 예시는 이 `docs/` 폴더로 분리해, 필요한 문서만 추가로 읽을 수 있게 합니다.
...
- 루트 `AGENTS.md`에는 모든 작업에 적용되는 공통 기준, 프로젝트별 기본값·지도, 문서 링크를 둡니다.
