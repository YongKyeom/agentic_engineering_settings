...
- 병합은 `git merge --no-ff`로 한다. 기능 단위가 머지 커밋으로 보이게 하고, 특별한 충돌 정리 사유가 없으면 `rebase`나 fast-forward 병합으로 이 이력을 펴지 않는다.
- **병합 후 삭제 대상은 워커 위임용 단명 브랜치·워크트리다.** 작업을 워커에게 배분할 때 작업 단위를 물리적으로 분리하려고 만든 브랜치·워크트리는 병합 즉시 지운다 — 지우지 않으면 위임 횟수만큼 로컬 브랜치가 쌓인다.
- **주제 브랜치(`feature/`·`fix/`)는 사용자가 삭제를 지정할 때까지 삭제 금지다.** `dev` 병합을 마쳤더라도, 중간 집결 병합(예: 진행 중 성과를 상위 feature 브랜치로 모으는 병합)을 했더라도 에이전트가 스스로 지우지 않는다.
